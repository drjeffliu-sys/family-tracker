...
async function apiGet() {
  ...
  if (!data.ok) throw new Error("讀取失敗");
...
async function apiAdd(entry) {
  await jsonp(API_URL + "?action=add&payload=" + encodeURIComponent(JSON.stringify(entry)));
}

async function apiDelete(id) {
  await jsonp(API_URL + "?action=delete&id=" + encodeURIComponent(id));
}
